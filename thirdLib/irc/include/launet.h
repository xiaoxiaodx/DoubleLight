...
#define LAUMSG_SMCHANGE_CLIENT_SHOW    33     //智能分析区域发送改变通知客服端
#define LAUMSG_ALARM_IN_SHUT           34     //探头报警报警停止 LAUMSG_ALARM 是探头报警开始

#define LAUMSG_DXHJ_CHATMSG            35     //DXHJ聊天消息
...
#define LAUMSG_DISPDVDINFO             906    //DXHJ通知获取并显示光盘信息
#define LAUMSG_DXSERVERRECORD          907    //DXHJ server record status
#define LAUMSG_DXSHOWEVIDENCE          908    //DXHJ 证据展示状态消息通知
...
#define LAUMSG_SMART_TARGETCOUNT       1016   //车人流量统计数量报警
#define LAUMSG_PEOPLECOUNT_SZFRS       1017   //SZFRS人流量统计数量报警
#define LAUMSG_CURRENTLOOP_BJJY        1018   //BJJY电流环信息和报警  
#define LAUMSG_ALARMIN_BJJY            1019   //BJJY探头报警
#define LAUMSG_SXKD_JPEGCALLBACK       1020   //SXKD jped回传报警
...
/***********************************************/
#define MAX_VIDEO_DISPLAY_RECT    4           //可扩展的预览区域最大数()
/***********************************************/
...
enum 
{
    DIARY_DOWNLOAD_STATUS_START = 1,     // 开始下载
    DIARY_DOWNLOAD_STATUS_STOP ,         // 下载正常结束    //线程结束            
    DIARY_DOWNLOAD_STATUS_CONTINUE,      // 下载正在继续
    DIARY_DOWNLOAD_STATUS_PARAM_ERR,     // 参数错误        //线程结束 
    DIARY_DOWNLOAD_STATUS_CONNECT,       // 正在连接服务器
    DIARY_DOWNLOAD_STATUS_SOCKET_ERR,    // 网络传输错误    //线程结束 
    DIARY_DOWNLOAD_STATUS_USER_ERR,      // 服务器拒绝 用户名密码错误等  //线程结束   
};

typedef struct 
{
    int nStatus1;                // 参考上面enum定义
    int nStatus2;                // nStatus1更详细的状态
...
{
    GETERROR        = 0,            //错误
    GETSDSTATE      = 1<<0,         //获取SD卡状态
    GET3GSIGNAL     = 1<<1,         //获取3G信号强度
    GETALARMSTATE   = 1<<2,         //获取报警状态
};

enum VSNET_NTFILE_ERR
{
	VSNET_NTFILE_OK = 0,
	VSNET_NTFILE_ERR_CREATE_SOCKET,       //套接字创建错误
	VSNET_NTFILE_ERR_BIND_SOCKET,         //套接字绑定错误
	VSNET_NTFILE_ERR_LISTEN_SOCKET,       //套接字监听错误
	VSNET_NTFILE_ERR_IS_RUN,              //服务已经启动
	VSNET_NTFILE_ERR_NO_SUPPORT,          //不支持该服务
	VSNET_NTFILE_ERR_NO_LISTENCLIENT,     //必须先调用 VSNET_StartListenClient
	VSNET_NTFILE_ERR_NO_MEMORY,           //内存不足
};
...
enum VSNET_CLIENT_LASTERR
{
    ...
    ERR_LINKFAIL        = -2,       //连接失败
    ERR_STREAMHEAD      = -3,       //错误的数据流头信息
    ERR_CALL_ORDER      = -4,       //调用数序错误
    ERR_OPENFILE        = -5,       //打开文件失败
    ERR_FILESIZE        = -6,       //文件大小错误    
    ERR_BUFFER_FULL     = -7,       //缓冲区满
    ERR_CREATEFILE      = -8,       //创建文件失败
    ERR_DISCONNECTED    = -9,       //服务器断开连接
    ERR_BINDPORT        = -10,      //端口绑定错误
    ERR_ALLOCMEM        = -11,      //分配内存失败
    ERR_USERORPASSWD    = -12,      //用户名或密码错误
    ERR_USEROVER        = -13,      //通道用户满员
    ERR_CHNOTEXIST      = -14,      //服务器没有指定通道
    ERR_REFUSED         = -15,      //服务器拒绝
    ERR_RECVDATA        = -16,      //接收数据错误
    ERR_SENDDATA        = -17,      //发送数据错误
    ERR_ITSUSEROVER     = -18,      //抓拍用户满员
};
