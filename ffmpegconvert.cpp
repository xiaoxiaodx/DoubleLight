...
void FfmpegConvert::rgb32ToH264(QImage img,QByteArray &arr,bool &gotpic)
{
...
#if 0
    char infile[] = "wocao.png";
    char outfile[] = "out.264";
    char outfile1[] = "out.yuv";
    ...
    if (!fpin)
    {
        qDebug() << infile << "open infile failed!" << endl;
    ...
    if (!fpout)
    {
        qDebug() << "open outfile failed!" << endl;

    }

    FILE *fpout1 = fopen(outfile1, "wb");
    if (!fpout)
    {
        qDebug() << "open outfile1 failed!" << endl;
    ...
    if (!codec)
    {
        qDebug() << "avcodec_find_encoder AV_CODEC_ID_H264 failed!" << endl;
    ...
    qDebug()<<" yuvFrame:"<<yuvFrame->data[100];

    fwrite((unsigned char*)yuvFrame->data, 1, yuvFrame->linesize[0], fpout1);
